/**
 * @module Entregados Laureles
 */
import React, { PropTypes } from 'react';
import { DataTable, TableHeader } from 'react-mdl';
/**
 * Laureles que se han entregado al colaborador.
 */
const laurelesEntregados = ({rows}) => {
	let rowsLaureles = rows.map((laurel, index) => {
		return (
			<tr key={index} onClick={() => { window.document.location = "#/entrego-laurel/resumen/" + laurel.idLaurel } }>
...
laurelesEntregados.propTypes = {
	rows: PropTypes.array.isRequired
};

export default laurelesEntregados;
