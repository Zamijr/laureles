...
export default (state = initialState, action) => {
	...
	case LAURELES_ENTREGADOS_SUCCESS: {
		return state.set('laurelesEntregados',  new Immutable.List(action.laureles));
	}
 ...
 */
export const listarLaurelesEntregados = (idColaborador, token) => {
	return (dispatch) => {
		dispatch(beginAjaxCall());
		return LaurelApi.listarLaurelesEntregados(idColaborador, token).then(response => {
